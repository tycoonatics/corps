...
if not df.empty:
    ...
    with tab_streaks:
        ...
        for uid in corp_df['UID'].unique():
            ...
            # 1. Activity Streak (Logged any data)
            act_streak = 0
            for w in distinct_weeks:
                if w in logged_dates: act_streak += 1
                else: break
            
            # 2. Donation Streak (DC > 0)
            don_streak = 0
            for w in distinct_weeks:
                if w in logged_dates:
                    if p_logs[p_logs['Date'] == w]['DC'].values[0] > 0:
                        don_streak += 1
                    else: break
                else: break
            ...
            streak_data.append({
                "Player Name": p_name,
                "Activity Streak": act_streak,
                "Donation Streak": don_streak,
                "Status": "✅ Active" if act_streak > 0 else "💤 Inactive"
            })
        
        streak_df = pd.DataFrame(streak_data).sort_values(by="Activity Streak", ascending=False)
        
        # Display as a clean table with icons
        st.write("### 🎖️ Top Contributors")
        st.dataframe(
            streak_df, 
            column_config={
                "Activity Streak": st.column_config.NumberColumn("Weeks Active", format="🔥 %d Weeks"),
                "Donation Streak": st.column_config.NumberColumn("Donation Streak", format="💚 %d Weeks"),
            },
            hide_index=True, 
    ...
    with tab_admin:
        ...
        with st.form("admin_form", clear_on_submit=True):
            ...
            if in_uid in player_map: f1.success(f"Mapping to: **{player_map[in_uid]}**")
