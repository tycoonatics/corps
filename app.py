...
if not df.empty:
    ...
    with tab_streaks:
        ...
        st.dataframe(
            pd.DataFrame(streak_data).sort_values(by="Activity Streak", ascending=False), 
            column_config={
                "Activity Streak": st.column_config.NumberColumn("Weeks Active", format="🔥 %d"),
                "Donation Streak": st.column_config.NumberColumn("Donation Streak", format="💚 %d"),
            },
            hide_index=True, 
